...
| Action | Function | Event | Current state |
|---|---|---|---|
| Join | `joinRoom` | `as:Join` | latest membership = `as:Join` → member |
| Leave | `leaveRoom` | `as:Leave` | latest membership = `as:Leave` → not a member |
| Set role(s) | `setMyRole` | `as:Update` + `sioc:has_function` | latest = current role set |
