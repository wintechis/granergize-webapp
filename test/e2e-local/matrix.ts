...
async function runLane(
  ...
  // own `webServer` (per the config), so the lane is just `playwright test`. Tee to a
  // per-lane log so the combined report can pull each lane's summary line.
  const { code } = await new Deno.Command("sh", {
    args: ["-c", `npx playwright test --project=local > ${log} 2>&1`],
    env,
  }).output();
...
function summarize(log: string): string {
  const lines = log.split("\n")
    ...
    .map((l) => l.trim());
  return lines.length ? lines.join(" / ") : "(no summary line — lane likely crashed)";
}
