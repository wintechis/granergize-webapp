...
We introduce the concept of Data Rooms, which enable a a group of stakeholders to identify data sharing possibilities.
We present a Solid-based application that enables bidirectional sharing of building energy data.
The system also supports aggregated views that expose only computed statistics without revealing underlying fine-grained building data. 
\end{abstract}
...
The stakeholder landscape's complexity makes it difficult to access all the relevant data for a comprehensive view of energy consumption.
...
To address the trust challenges, we have developed an ontology that provides a unified data model for energy consumption data, as well as a WebApp for visualizing and sharing this data, stored on the user's Solid Pods.
...
The data model builds upon established ontologies -- including RealEstateCore for building classifications and Schema.org for organizational entities -- and introduces domain-specific terms via our GRANERGIZE vocabulary (\mintinline{turtle}{gra:}).
...
Our WebApp addresses this through aggregated views -- snapshots containing only computed values.
...
This paper introduces the concept of Data Rooms -- a structured process for stakeholders to identify and formalize data sharing requirements, establishing trust and enabling partner discovery before any data is exchanged. Additionally, a Solid-based Web application that addresses these challenges through decentralized data sharing while preserving data sovereignty.
...
We acknowledge funding from the German Federal Ministry for Economic Affairs and Energy for the project GRANERGIZE (FKZ 01IF23286N).
